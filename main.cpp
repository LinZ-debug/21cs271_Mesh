...
void InitMenu() {
	...
	// glutAddMenuEntry("Select Mode", 98);
	//glutAddMenuEntry("Delete selected vertex", 97);
...
void MenuCallback(int value) {
	...
	case COLORSMOOTHSHADED:
		...
		break;
	/*case 97:
		cout <<"delete vertex id: "<< currSelectedVertex << endl;
		// DeleteSelectedVertex(currSelectedVertex); 
		glutPostRedisplay();
		break;
	case 98:
		currentMode = Selection; break;*/
	case 99: 
		exit(0); break;
	default: 
		displayMode = value;
...
void MouseFunc(int button, int state, int x, int y) {
	...
	if (currentMode == Selection && state == GLUT_UP)
	{
		...
		cout << "selected vertex id: "<< currSelectedVertex << endl;
		lastX = lastY = 0;
		glutPostRedisplay();
	}
}
...
// delete one vertex by click
void DeleteSelectedVertex(int vertex) {
	VertexList vList = mesh.Vertices();
	Vertex* v = vList[vertex];
	cout <<"delete vertex position:"<< v->Position() << endl;
	OneRingHEdge ring(v);
	HEdge* e = NULL;
	while ( e = ring.NextHEdge()) {
		e->SetFlag(0);
		e->Twin()->SetFlag(0);
		e->Next()->SetFlag(0);
	} 
...
void main(int argc, char **argv) {
	...
	/************************************************************************/
	/* activate the following code if you finish the corresponding functions*/
	char* name = argc >= 2? argv[argc-1]:argv[0];
	cout << "Name of Obj: " << name << endl;
	mesh.DisplayMeshInfo();
	/************************************************************************/
	/*for (size_t i = 0; i < mesh.vList.size(); i++) {
		cout << "index=" << (mesh.vList[i])->Index() << " i=" << i << endl;
	}*/
	glutMainLoop();
