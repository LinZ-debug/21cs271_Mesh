...
private:
    ...
    bool valid;
	double curv;		// vertex curvature
public:
	vector<HEdge*> adjHEdges; // for reading object only, do not use it in other place

	// constructors
	Vertex() : he(NULL), flag(0), valid(true), curv(0) { }
	Vertex(const Vector3d & v) : he(NULL), position(v), flag(0), valid(true), curv(0) { }
	Vertex(double x, double y, double z) : he(NULL), position(x,y,z), flag(0), valid(true), curv(0) { }
	...
	const double & Curv() const { return curv; }
	...
	double SetCurv(int curv) { return Vertex::curv = curv; }
