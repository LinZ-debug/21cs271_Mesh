...
private:
	...
	int * rowIndex;				// row indicex of non-zero entries
...
public:
	...
	void SortMatrix()
	{
		...
		//cout << "row size:"<< sizeof(rowIndex) << endl;
		sort(elements.begin( ), elements.end( ), MatrixElement::order);
		// cout << "elements" << elements << endl;
		// record rows of non-zero elements: rowIndex[ele.row+1]=index+1 otherwise 0
		for (int i=0; i<m+1; i++)
			rowIndex[i] = 0;
		for (int i = 0; i < (int)elements.size(); i++) {
			//cout << "i=" << i << "\trow=" << elements[i].row << endl;
	...
	// helper function
	// i-th row elements devide by scalar
	void divideByRow(int row, double scalar) {
		int i = row;
		for (int j = rowIndex[i]; j < rowIndex[i + 1]; j++) {
			elements[j].value /= scalar;
		}
	}
	/////////////////////////////////////
	// function Multiply
	...
	void Multiply(double* xIn, double* xOut)
	{
		...
		{
			double sum = 0;
			for (int j=rowIndex[i]; j<rowIndex[i+1]; j++)  //all elements in i-th column
